defmodule Aion.RoomChannel.Room do
  ...
  def new(room_id, state \\ []) do
    case state do
      ...
      _ ->
        Logger.error(fn -> "Unexpected state passed to Room.new: #{state}" end)
        %Room{room_id: room_id}
    end
  end
  ...
  def award_user(room, username, user_id, amount \\ 1) do
    ...
    update_in(
      room,
      [Access.key!(:users), Access.key!(username)],
      &UserRecord.update_score(&1, amount)
    )
  end
