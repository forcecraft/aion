defmodule Aion.QuestionChronicle do
  ...
  def should_change?(room_id) do
    last_changed = Agent.get(__MODULE__, fn chronicle ->
      Map.get(chronicle, room_id)
    end)

    get_current_time() >= last_changed + @question_timeout_micro
  end

  @doc "Updates question change's timestamp"
  def update_last_change(room_id) do
    Agent.update(__MODULE__, &Map.put(&1, room_id, get_current_time()))
  end
